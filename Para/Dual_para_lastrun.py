...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\documentos Alexa\\Dual_para_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "Circulo" ---
    ...
    # --- Prepare to start Routine "Welcome_screen_" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Welcome_screen_.started', globalClock.getTime())
    # keep track of which components have finished
    Welcome_screen_Components = [Welcome]
    for thisComponent in Welcome_screen_Components:
    ...
    # --- Ending Routine "Welcome_screen_" ---
    for thisComponent in Welcome_screen_Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Welcome_screen_.stopped', globalClock.getTime())
    ...
    # keep track of which components have finished
    InstruccionsComponents = [Instrucccions]
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        # if Instrucccions is stopping this frame...
        if Instrucccions.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Instrucccions.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                Instrucccions.tStop = t  # not accounting for scr refresh
                Instrucccions.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Instrucccions.stopped')
                # update status
                Instrucccions.status = FINISHED
                Instrucccions.setAutoDraw(False)
    ...
    # --- Prepare to start Routine "Circulo" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Circulo.started', globalClock.getTime())
    # keep track of which components have finished
    CirculoComponents = [Circle]
    for thisComponent in CirculoComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        if Circle.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Circle.tStartRefresh + 5.0-frameTolerance:
                # keep track of stop time/frame for later
    ...
    # --- Ending Routine "Circulo" ---
    for thisComponent in CirculoComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Circulo.stopped', globalClock.getTime())
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
